...
<td><button type="button" class="btn btn-info btn-xs" data-toggle="modal" onclick="borrowbook(<%= bean.getBid() %>,<%= bean.getStatus() %>)" >借阅</button>   	</td>
    ...
    function borrowbook(bid,status) {
        <%
           BookDao bookdao = new BookDao();
           int booksnum=0;
           booksnum = bookdao.selectBooksByAid(aid,1);
        %>
        var num = <%=booksnum%>;
        //confirm(num);
        if( num >= 3){
