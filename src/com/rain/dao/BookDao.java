...
public class BookDao {
	 ...
	 * @param status
	 */
	public void addBook(String card, String name, String type, String autho, String press, int status) {
		// TODO Auto-generated method stub
		Connection conn = DBUtil.getConnectDb();
		String sql = "insert  into book(card,name,type,autho,press,status) values(?,?,?,?,?,?)";
		int rs = 0;
	...
	public ArrayList<HistoryBean> get_overHistoryListInfo2(int status){
		ArrayList<HistoryBean> tag_Array = new ArrayList<HistoryBean>();
		Connection conn = DBUtil.getConnectDb();
		String sql = "select * from history where status='"+status+"'";
		PreparedStatement stm = null;
		...
		String DATE1 = f.format(c.getTime());
		try {
			stm = conn.prepareStatement(sql);
			rs = stm.executeQuery();
			while(rs.next()){
				...
				HistoryBean tag = new HistoryBean();
				String DATE2 = rs.getString("endtime");
	 ...
	 * 修改图书的信息，bid作为条件，
	 */
	public void updateBook(int bid, String card, String name, String type, String autho, String press) {
		// TODO Auto-generated method stub
		Connection conn = DBUtil.getConnectDb();
		String sql = "update book set name=?,card=?,type=?,autho=?,press=? where bid=?";
		PreparedStatement stm = null;
		try {
			stm = conn.prepareStatement(sql);
	...
	public void borrowBook(int bid, AdminBean adminbean) {
		...
		Calendar c = Calendar.getInstance();
		String begintime=f.format(c.getTime());
	...
	public  void updateBookStatus(int bid,int status){
		Connection conn = DBUtil.getConnectDb();
	...
	public int selectBookByHid(int hid){
		Connection conn = DBUtil.getConnectDb();
		int bid=0;
		String sql = "SELECT bid FROM history where hid=?";
		PreparedStatement stm = null;
		ResultSet rs = null;
	...
	public int selectBooksByAid(String aid,int status){
		System.out.println("aid="+aid);
		System.out.println("status="+status);

		Connection conn = DBUtil.getConnectDb();
		int booksnum = 0;
		String sql = "SELECT count(*) as num  FROM history WHERE aid=? and status=?";
		PreparedStatement stm = null;
		ResultSet rs = null;
